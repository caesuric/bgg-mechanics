...
def main():
    ...
    headers = []
    with open('categories_data.csv') as data:
        ...
        for row in csvreader:
            ...
            break
    data = pandas.read_csv('categories_data.csv', dtype=dtypes)
    columns = []
    for i in range(84):
        columns.append('bayes_rating')
    Y = data[columns].values.reshape(-1, 84)
    data = data.drop(columns=['bayes_rating'])
    X = data.values.reshape(-1,84)
